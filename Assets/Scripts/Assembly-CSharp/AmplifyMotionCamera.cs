[global::UnityEngine.AddComponentMenu("")]
[global::UnityEngine.RequireComponent(typeof(global::UnityEngine.Camera))]
public class AmplifyMotionCamera : global::UnityEngine.MonoBehaviour
{
	internal AmplifyMotionEffectBase Instance;

	internal global::UnityEngine.Matrix4x4 PrevViewProjMatrix;

	internal global::UnityEngine.Matrix4x4 ViewProjMatrix;

	internal global::UnityEngine.Matrix4x4 InvViewProjMatrix;

	internal global::UnityEngine.Matrix4x4 PrevViewProjMatrixRT;

	internal global::UnityEngine.Matrix4x4 ViewProjMatrixRT;

	internal global::UnityEngine.Transform Transform;

	private bool m_linked;

	private bool m_initialized;

	private bool m_starting = true;

	private bool m_autoStep = true;

	private bool m_step;

	private bool m_overlay;

	private global::UnityEngine.Camera m_camera;

	private int m_prevFrameCount;

	private global::System.Collections.Generic.HashSet<AmplifyMotionObjectBase> m_affectedObjectsTable = new global::System.Collections.Generic.HashSet<AmplifyMotionObjectBase>();

	private AmplifyMotionObjectBase[] m_affectedObjects;

	private bool m_affectedObjectsChanged = true;

	private const global::UnityEngine.Rendering.CameraEvent m_renderCBEvent = global::UnityEngine.Rendering.CameraEvent.BeforeImageEffects;

	private global::UnityEngine.Rendering.CommandBuffer m_renderCB;

	public bool Initialized => m_initialized;

	public bool AutoStep => m_autoStep;

	public bool Overlay => m_overlay;

	public global::UnityEngine.Camera Camera => m_camera;

	public void RegisterObject(AmplifyMotionObjectBase obj)
	{
		m_affectedObjectsTable.Add(obj);
		m_affectedObjectsChanged = true;
	}

	public void UnregisterObject(AmplifyMotionObjectBase obj)
	{
		m_affectedObjectsTable.Remove(obj);
		m_affectedObjectsChanged = true;
	}

	private void UpdateAffectedObjects()
	{
		if (m_affectedObjects == null || m_affectedObjectsTable.Count != m_affectedObjects.Length)
		{
			m_affectedObjects = new AmplifyMotionObjectBase[m_affectedObjectsTable.Count];
		}
		m_affectedObjectsTable.CopyTo(m_affectedObjects);
		m_affectedObjectsChanged = false;
	}

	public void LinkTo(AmplifyMotionEffectBase instance, bool overlay)
	{
		Instance = instance;
		m_camera = GetComponent<global::UnityEngine.Camera>();
		m_camera.depthTextureMode |= global::UnityEngine.DepthTextureMode.Depth;
		InitializeCommandBuffers();
		m_overlay = overlay;
		m_linked = true;
	}

	public void Initialize()
	{
		m_step = false;
		UpdateMatrices();
		m_initialized = true;
	}

	private void InitializeCommandBuffers()
	{
		ShutdownCommandBuffers();
		m_renderCB = new global::UnityEngine.Rendering.CommandBuffer();
		m_renderCB.name = "AmplifyMotion.Render";
		m_camera.AddCommandBuffer(global::UnityEngine.Rendering.CameraEvent.BeforeImageEffects, m_renderCB);
	}

	private void ShutdownCommandBuffers()
	{
		if (m_renderCB != null)
		{
			m_camera.RemoveCommandBuffer(global::UnityEngine.Rendering.CameraEvent.BeforeImageEffects, m_renderCB);
			m_renderCB.Release();
			m_renderCB = null;
		}
	}

	private void Awake()
	{
		Transform = base.transform;
	}

	private void OnEnable()
	{
		AmplifyMotionEffectBase.RegisterCamera(this);
	}

	private void OnDisable()
	{
		m_initialized = false;
		ShutdownCommandBuffers();
		AmplifyMotionEffectBase.UnregisterCamera(this);
	}

	private void OnDestroy()
	{
		if (Instance != null)
		{
			Instance.RemoveCamera(m_camera);
		}
	}

	public void StopAutoStep()
	{
		if (m_autoStep)
		{
			m_autoStep = false;
			m_step = true;
		}
	}

	public void StartAutoStep()
	{
		m_autoStep = true;
	}

	public void Step()
	{
		m_step = true;
	}

	private void Update()
	{
		if (m_linked && Instance.isActiveAndEnabled)
		{
			if (!m_initialized)
			{
				Initialize();
			}
			if ((m_camera.depthTextureMode & global::UnityEngine.DepthTextureMode.Depth) == 0)
			{
				m_camera.depthTextureMode |= global::UnityEngine.DepthTextureMode.Depth;
			}
		}
	}

	private void UpdateMatrices()
	{
		if (!m_starting)
		{
			PrevViewProjMatrix = ViewProjMatrix;
			PrevViewProjMatrixRT = ViewProjMatrixRT;
		}
		global::UnityEngine.Matrix4x4 worldToCameraMatrix = m_camera.worldToCameraMatrix;
		global::UnityEngine.Matrix4x4 gPUProjectionMatrix = global::UnityEngine.GL.GetGPUProjectionMatrix(m_camera.projectionMatrix, renderIntoTexture: false);
		ViewProjMatrix = gPUProjectionMatrix * worldToCameraMatrix;
		InvViewProjMatrix = global::UnityEngine.Matrix4x4.Inverse(ViewProjMatrix);
		global::UnityEngine.Matrix4x4 gPUProjectionMatrix2 = global::UnityEngine.GL.GetGPUProjectionMatrix(m_camera.projectionMatrix, renderIntoTexture: true);
		ViewProjMatrixRT = gPUProjectionMatrix2 * worldToCameraMatrix;
		if (m_starting)
		{
			PrevViewProjMatrix = ViewProjMatrix;
			PrevViewProjMatrixRT = ViewProjMatrixRT;
		}
	}

	public void FixedUpdateTransform(AmplifyMotionEffectBase inst, global::UnityEngine.Rendering.CommandBuffer updateCB)
	{
		if (!m_initialized)
		{
			Initialize();
		}
		if (m_affectedObjectsChanged)
		{
			UpdateAffectedObjects();
		}
		for (int i = 0; i < m_affectedObjects.Length; i++)
		{
			if (m_affectedObjects[i].FixedStep)
			{
				m_affectedObjects[i].OnUpdateTransform(inst, m_camera, updateCB, m_starting);
			}
		}
	}

	public void UpdateTransform(AmplifyMotionEffectBase inst, global::UnityEngine.Rendering.CommandBuffer updateCB)
	{
		if (!m_initialized)
		{
			Initialize();
		}
		if (global::UnityEngine.Time.frameCount <= m_prevFrameCount || (!m_autoStep && !m_step))
		{
			return;
		}
		UpdateMatrices();
		if (m_affectedObjectsChanged)
		{
			UpdateAffectedObjects();
		}
		for (int i = 0; i < m_affectedObjects.Length; i++)
		{
			if (!m_affectedObjects[i].FixedStep)
			{
				m_affectedObjects[i].OnUpdateTransform(inst, m_camera, updateCB, m_starting);
			}
		}
		m_starting = false;
		m_step = false;
		m_prevFrameCount = global::UnityEngine.Time.frameCount;
	}

	public void RenderReprojectionVectors(global::UnityEngine.RenderTexture destination, float scale)
	{
		m_renderCB.SetGlobalMatrix("_AM_MATRIX_CURR_REPROJ", PrevViewProjMatrix * InvViewProjMatrix);
		m_renderCB.SetGlobalFloat("_AM_MOTION_SCALE", scale);
		global::UnityEngine.RenderTexture tex = null;
		m_renderCB.Blit(new global::UnityEngine.Rendering.RenderTargetIdentifier(tex), destination, Instance.ReprojectionMaterial);
	}

	public void PreRenderVectors(global::UnityEngine.RenderTexture motionRT, bool clearColor, float rcpDepthThreshold)
	{
		m_renderCB.Clear();
		m_renderCB.SetGlobalFloat("_AM_MIN_VELOCITY", Instance.MinVelocity);
		m_renderCB.SetGlobalFloat("_AM_MAX_VELOCITY", Instance.MaxVelocity);
		m_renderCB.SetGlobalFloat("_AM_RCP_TOTAL_VELOCITY", 1f / (Instance.MaxVelocity - Instance.MinVelocity));
		m_renderCB.SetGlobalVector("_AM_DEPTH_THRESHOLD", new global::UnityEngine.Vector2(Instance.DepthThreshold, rcpDepthThreshold));
		m_renderCB.SetRenderTarget(motionRT);
		m_renderCB.ClearRenderTarget(clearDepth: true, clearColor, global::UnityEngine.Color.black);
	}

	public void RenderVectors(float scale, float fixedScale, global::AmplifyMotion.Quality quality)
	{
		if (!m_initialized)
		{
			Initialize();
		}
		float nearClipPlane = m_camera.nearClipPlane;
		float farClipPlane = m_camera.farClipPlane;
		global::UnityEngine.Vector4 value = default(global::UnityEngine.Vector4);
		if (AmplifyMotionEffectBase.IsD3D)
		{
			value.x = 1f - farClipPlane / nearClipPlane;
			value.y = farClipPlane / nearClipPlane;
		}
		else
		{
			value.x = (1f - farClipPlane / nearClipPlane) / 2f;
			value.y = (1f + farClipPlane / nearClipPlane) / 2f;
		}
		value.z = value.x / farClipPlane;
		value.w = value.y / farClipPlane;
		m_renderCB.SetGlobalVector("_AM_ZBUFFER_PARAMS", value);
		m_renderCB.SetGlobalMatrix("_AM_MATRIX_PREV_VP", PrevViewProjMatrixRT);
		if (m_affectedObjectsChanged)
		{
			UpdateAffectedObjects();
		}
		for (int i = 0; i < m_affectedObjects.Length; i++)
		{
			if ((m_camera.cullingMask & (1 << m_affectedObjects[i].gameObject.layer)) != 0)
			{
				m_affectedObjects[i].OnRenderVectors(m_camera, m_renderCB, m_affectedObjects[i].FixedStep ? fixedScale : scale, quality);
			}
		}
	}
}
