...
{
	public class ParticleManager : global::UnityEngine.MonoBehaviour
	{
		protected global::System.Collections.Generic.List<global::UnityEngine.ParticleSystem[]> particlePrefabs;
		...
		public void Init()
		{
			particlePrefabs = new global::System.Collections.Generic.List<global::UnityEngine.ParticleSystem[]>();
			int childCount = base.transform.childCount;
			for (int i = 0; i < childCount; i++)
			{
				particlePrefabs.Add(base.transform.GetChild(i).GetComponentsInChildren<global::UnityEngine.ParticleSystem>());
			}
			if (disableChildrenAtStart)
			{
				for (int j = 0; j < particlePrefabs.Count; j++)
				{
					particlePrefabs[j][0].gameObject.SetActive(value: false);
				}
			}
			initialized = true;
		}
		...
		public string GetCurrentPrefabName(bool shorten = false)
		{
			...
			if (shorten)
			{
				...
				for (int i = 0; i < prefabNameUnderscoreCountCutoff; i++)
				{
					...
					if (num == 0)
					{
						global::UnityEngine.MonoBehaviour.print("Iteration of underscore not found.");
						break;
					}
				}
